...
/** Genera la key de un ingrediente a partir del nombre.
 * - Minúsculas
 * - Espacios → guiones
 * - Quita artículos y preposiciones (el, la, de, en, con, etc.)
...
function parseIngredientLine(line) {
  ...
  if (right.toLowerCase() === "al gusto") {
    return { name, key: nameToKey(name), qty: null, unit: "al gusto" };
  }
  ...
  if (parts.length === 1) {
    ...
    return { name, key: nameToKey(name), qty: parseQty(parts[0]), unit: "" };
  }
  const qtyRaw = parts[0];
  const unit = parts.slice(1).join(" ");

  return { name, key: nameToKey(name), qty: parseQty(qtyRaw), unit };
}
...
function parseRecipesFromMd(md) {
  ...
  for (const part of parts) {
    ...
    if (!id || id.includes("**") || title === "(Nueva receta)") {
      console.warn(`⚠️ Receta sin id o plantilla: "${title}" (se omite)`);
      continue;
    }
